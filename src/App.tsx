...
const App: React.FC = () => {
    ...
    return (
        <div className="mx-auto px-10 py-12 font-sans md:px-12 md:py-20 lg:px-[25%] lg:py-0">
            <div className="lg:flex lg:justify-between lg:gap-4">
                <header className="lg:sticky lg:top-0 lg:flex lg:max-h-screen lg:w-1/2 lg:flex-col lg:justify-between lg:py-24">
                    <div>
                        ...
                        <h1 className="text-4xl font-bold tracking-tight sm:text-5xl title">{name}</h1>
                        <h2 className="mt-3 text-lg font-medium tracking-tight title sm:text-xl">{title}</h2>
                        <p className="mt-4 max-w-xs leading-normal texto">{bio}</p>
                                ...
                                {navItems.map((item) => (
                                    <li key={item.id}>
                                        <a className="group flex items-center py-3" href={`#${item.id}`} onClick={handleNavClick}>
                                            ...
                                            <span className={`nav-text text-xs font-bold uppercase tracking-widest nav group-hover:title ${activeSection === item.id ? 'text-[#AB3E6E]' : ''}`}>{item.label}</span>
                                        </a>
                                    </li>
                                ))}
                            </ul>
                    ...
                    <ul className="ml-1 mt-8 flex items-center" aria-label="Social media">
                        <li className="mr-5 shrink-0"><a href={socialLinks.github} target="_blank" rel="noreferrer noopener" aria-label="GitHub"><GithubIcon className="h-6 w-6 nav hover:text-[#AB3E6E] transition-colors" /></a></li>
                        <li className="mr-5 shrink-0"><a href={socialLinks.linkedin} target="_blank" rel="noreferrer noopener" aria-label="LinkedIn"><LinkedinIcon className="h-6 w-6 nav hover:text-[#AB3E6E] transition-colors" /></a></li>
                        <li className="mr-5 shrink-0"><a href={`mailto:${socialLinks.email}`} aria-label="Email"><MailIcon className="h-6 w-6 nav hover:text-[#AB3E6E] transition-colors" /></a></li>
                        <ThemeToggle />
                    </ul>
                </header>
                ...
                <main id="content" className="pt-24 lg:w-1/2 lg:py-24">
                    ...
                    <Section ref={sectionRefs.about} id="about" title="Acerca de">
                        <p className="texto leading-relaxed">{about}</p>
                    </Section>
                    {/* Fix: Pass id and ref to Section component and remove wrapper div to fix missing prop errors. */}
                    <Section ref={sectionRefs.skills} id="skills" title="Habilidades">
                        <ul className="flex flex-wrap gap-2">
                            {skills.map((skill) => (
                                <li key={skill}>
                                    <div className="flex items-center rounded-full chip px-3 py-1 text-sm font-medium leading-5">{skill}</div>
                                </li>
                            ))}
                        </ul>
                    </Section>
